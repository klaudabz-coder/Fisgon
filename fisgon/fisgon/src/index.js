...
// Create webhook clients only if tokens are available
let startLogs = null;
let shardLogs = null;

if (webhook.startLogs.id && webhook.startLogs.token) {
...
if (webhook.shardLogs.id && webhook.shardLogs.token) {
...
if (webhook.consoleLogs.id && webhook.consoleLogs.token) {
...
if (webhook.warnLogs.id && webhook.warnLogs.token) {
...
process.on('unhandledRejection', error => {
    ...
    if(consoleLogs) consoleLogs.send({
        username: 'Bot Logs',
        embeds: [embed],
    }).catch(() => {
