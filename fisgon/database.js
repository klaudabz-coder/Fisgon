    ...
    const caches = {
      ...
      adventure: new Map() // <--- AÑADIDO: Progreso del Atrapasueños
    };
      ...
      try {
        ...
        if (enabled) {
          ...
          await Promise.all([
            ...
            loadCollectionToCache('adventure', caches.adventure, keyFromDocEconomy) // <--- CARGAR
          ]);
    ...
    module.exports = {
      ...
      setLevelConfig(guildId, base, exponent) { const obj = { guild_id: guildId, base, exponent }; caches.level_config.set(guildId, obj); persistDoc('level_config', guildId, obj); },
      getLevelConfig(guildId) { return caches.level_config.get(guildId) || { base: 100, exponent: 1.5 }; },

      // --- NUEVAS FUNCIONES DE AVENTURA ---
      getAdventureCharge(guildId, userId) {
        const key = `${guildId}_${userId}`;
        const data = caches.adventure.get(key);
        return data ? data.charge : 0;
      },
      addAdventureCharge(guildId, userId, amount) {
        const key = `${guildId}_${userId}`;
        const cur = caches.adventure.get(key) || { guild_id: guildId, user_id: userId, charge: 0 };
        cur.charge = (cur.charge || 0) + amount;
        caches.adventure.set(key, cur);
        persistDoc('adventure', key, cur);
        return cur.charge;
      },
      setAdventureCharge(guildId, userId, amount) {
        const key = `${guildId}_${userId}`;
        const cur = caches.adventure.get(key) || { guild_id: guildId, user_id: userId };
        cur.charge = amount;
        caches.adventure.set(key, cur);
        persistDoc('adventure', key, cur);
      }
