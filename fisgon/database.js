...
module.exports = {
  ...
  getAdventureCharge(guildId, userId) { const key = `${guildId}_${userId}`; const data = caches.adventure.get(key); return data ? data.charge : 0; },
  ...
  getSets(guildId) {
    const out = [];
    for (const v of caches.card_sets.values()) if (v.guild_id === guildId) out.push(v);
    return out;
  },
